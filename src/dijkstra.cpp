                   ...
                   Graph::UndirectedGraph *map)
{
  ...
  {
    Graph::Node *node = *it;
    distances[node] = 1024 * 1024; // just a large number (hack)
    open.insert(node);
  }
  ...
  while (true)
  {
    ...
    for (; iter != edges->end(); iter++)
    {
      ...
      {
        distances[edge->get_y()] = std::min(distances[edge->get_y()],
                                            distances[edge->get_x()] + edge->get_weight());
        back_pointer[edge->get_y()] = edge->get_x();
      }
      else if (closed.find(edge->get_y()) != closed.end() && open.find(edge->get_x()) != open.end())
      {
        distances[edge->get_x()] = std::min(distances[edge->get_x()],
                                            distances[edge->get_y()] + edge->get_weight());
